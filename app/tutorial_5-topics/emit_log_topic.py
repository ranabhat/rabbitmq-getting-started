...
class Emitter():
    ...
    def return_random_routing_key(self):
        error = ["anonymous.info", "anonymous.warning", "kern.critical"]
        self.routing_key = random.choice(error)
    ...
    def send_message_to_queue(self):
        ...
        print(f"Sent {self.routing_key}:{new_message}")
...
if __name__ == '__main__':
    while True:
        try:
            emitter_instance = Emitter(hostname='rabbitmq-getting-started_rabbitmq_1', queue='', routing_key='info', queue_name='', exchange='topic_logs', exchange_type='topic')
            emitter_instance.declare_exchange()
            while True:
                emitter_instance.send_message_to_queue()
                emitter_instance.return_random_routing_key()
                time.sleep(1)
        except IOError as e:
            print(f"i/o error")
            time.sleep(2)
